...
const VIEW_RADIUS = 4;
const TILE = 92;
const ENCOUNTER_W = 760;
const ENCOUNTER_H = 520;
...
export class ExpeditionScene extends Phaser.Scene {
  ...
  private encounterBackdrop?: Phaser.GameObjects.Rectangle;
  ...
  create(): void {
    ...
    const top = this.add.rectangle(0, 0, 1, 132, 0x172019, 0.98).setOrigin(0).setDepth(20).setScrollFactor(0);
    ...
    const bottom = this.add.rectangle(0, 0, 1, 270, 0x172019, 0.985).setOrigin(0).setDepth(20).setScrollFactor(0);
    ...
    const makeBtn = (label: string, dx: number, dy: number, action: () => void) => {
      const bg = this.add.rectangle(0, 0, 84, 64, 0x354638, 1)
        .setStrokeStyle(2, 0x71816b).setInteractive({ useHandCursor: true }).setDepth(22).setScrollFactor(0);
      const tx = this.add.text(0, 0, label, {
        fontFamily: 'system-ui, sans-serif', fontSize: '24px', color: '#f4ead0', fontStyle: 'bold',
      ...
      bg.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, event: Phaser.Types.Input.EventData) => {
        ...
        if (!this.encounter) action();
      });
      return { bg, tx, dx, dy };
    };

    const controls = [
      makeBtn('↑', 0, -66, () => this.tryMove(0, -1)),
      makeBtn('←', -90, 0, () => this.tryMove(-1, 0)),
      makeBtn('↓', 0, 0, () => this.tryMove(0, 1)),
      makeBtn('→', 90, 0, () => this.tryMove(1, 0)),
    ];

    const returnBg = this.add.rectangle(0, 0, 210, 64, 0x5d4b37, 1)
      .setStrokeStyle(2, 0x9b815f).setInteractive({ useHandCursor: true }).setDepth(22).setScrollFactor(0);
    const returnText = this.add.text(0, 0, '返回营地', {
      fontFamily: 'system-ui, sans-serif', fontSize: '21px', color: '#fff1d2', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(23).setScrollFactor(0);
    returnBg.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, event: Phaser.Types.Input.EventData) => {
      event.stopPropagation();
      this.returnToCamp();
    });
    ...
    const layout = () => {
      ...
      top.setSize(width, 132);
      title.setPosition(26, 20);
      this.hudText?.setPosition(26, 66);
      bottom.setPosition(0, height - 270).setSize(width, 270);
      this.messageText?.setPosition(26, height - 244).setWordWrapWidth(Math.max(260, width - 52));

      const controlsCenterX = Math.min(width * 0.38, 360);
      const controlsCenterY = height - 88;
      controls.forEach(({ bg, tx, dx, dy }) => {
        bg.setPosition(controlsCenterX + dx, controlsCenterY + dy);
        tx.setPosition(controlsCenterX + dx, controlsCenterY + dy);
      });
      returnBg.setPosition(width - 135, height - 87);
      returnText.setPosition(width - 135, height - 87);

      this.layoutEncounterPanel();
      this.renderMap();
    };

    this.scale.on('resize', layout);
    this.input.on('pointerdown', (p: Phaser.Input.Pointer) => {
      if (!this.encounter) this.swipeStart = { x: p.x, y: p.y };
    });
    this.input.on('pointerup', (p: Phaser.Input.Pointer) => {
      if (!this.swipeStart || this.encounter) {
  ...
  private createEncounterPanel(): void {
    this.encounterBackdrop = this.add.rectangle(0, 0, 1, 1, 0x09100b, 0.72)
      .setOrigin(0).setDepth(39).setVisible(false).setInteractive().setScrollFactor(0);
    this.encounterBackdrop.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, event: Phaser.Types.Input.EventData) => event.stopPropagation());

    const panel = this.add.container(0, 0).setDepth(40).setVisible(false).setScrollFactor(0);
    const bg = this.add.rectangle(0, 0, ENCOUNTER_W, ENCOUNTER_H, 0x151c17, 0.995)
      .setStrokeStyle(3, 0x7b8d70).setInteractive();
    bg.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, event: Phaser.Types.Input.EventData) => event.stopPropagation());

    this.encounterTitle = this.add.text(-330, -220, '', {
      fontFamily: 'system-ui, sans-serif', fontSize: '30px', color: '#f5dfb0', fontStyle: 'bold',
    });
    this.encounterText = this.add.text(-330, -160, '', {
      fontFamily: 'system-ui, sans-serif', fontSize: '20px', color: '#d5deca', lineSpacing: 8, wordWrap: { width: 660 },
    });

    const attackBg = this.add.rectangle(-210, 170, 180, 60, 0x5a6948, 1)
      .setStrokeStyle(2, 0x8b9b70).setInteractive({ useHandCursor: true });
    this.attackText = this.add.text(-210, 170, '攻击', {
      fontFamily: 'system-ui, sans-serif', fontSize: '21px', color: '#fff2d2', fontStyle: 'bold',
    }).setOrigin(0.5);

    const healBg = this.add.rectangle(0, 170, 180, 60, 0x465a48, 1)
      .setStrokeStyle(2, 0x71856f).setInteractive({ useHandCursor: true });
    const healText = this.add.text(0, 170, '使用药剂', {
      fontFamily: 'system-ui, sans-serif', fontSize: '20px', color: '#e7eddc', fontStyle: 'bold',
    }).setOrigin(0.5);

    const fleeBg = this.add.rectangle(210, 170, 180, 60, 0x58483c, 1)
      .setStrokeStyle(2, 0x826d5b).setInteractive({ useHandCursor: true });
    const fleeText = this.add.text(210, 170, '撤离', {
      fontFamily: 'system-ui, sans-serif', fontSize: '20px', color: '#eadccc', fontStyle: 'bold',
    }).setOrigin(0.5);

    attackBg.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, event: Phaser.Types.Input.EventData) => {
      event.stopPropagation(); this.attackEnemy();
    });
    healBg.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, event: Phaser.Types.Input.EventData) => {
      event.stopPropagation(); this.useMedicine();
    });
    fleeBg.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, event: Phaser.Types.Input.EventData) => {
      event.stopPropagation(); this.fleeEncounter();
    });

    panel.add([bg, this.encounterTitle, this.encounterText, attackBg, this.attackText, healBg, healText, fleeBg, fleeText]);
    this.encounterPanel = panel;
    this.layoutEncounterPanel();
  }

  private layoutEncounterPanel(): void {
    const width = Math.max(1, this.scale.width);
    const height = Math.max(1, this.scale.height);
    this.encounterBackdrop?.setPosition(0, 0).setSize(width, height);
    const scale = Math.min(1, (width - 32) / ENCOUNTER_W, (height - 180) / ENCOUNTER_H);
    this.encounterPanel?.setScale(Math.max(0.62, scale));
    this.encounterPanel?.setPosition(width / 2, height / 2);
  }
  ...
  private renderMap(): void {
    ...
    for (let gy = -radius; gy <= radius; gy += 1) {
      for (let gx = -radius; gx <= radius; gx += 1) {
        ...
        const key = `${wx},${wy}`;
        const known = persistentRevealed.has(key);
        ...
        if (!known) {
          this.mapLayer.add(this.add.text(sx, sy, '?', {
            fontFamily: 'system-ui, sans-serif', fontSize: '24px', color: '#4b544c',
          }).setOrigin(0.5));
          continue;
        }
    ...
    this.mapLayer.add([
      ...
      this.add.text(centerX, centerY - 1, '@', {
        fontFamily: 'monospace', fontSize: '25px', color: '#2f2a22', fontStyle: 'bold',
      }).setOrigin(0.5),
    ]);
  }
  ...
  private inspectCurrentTile(): void {
    ...
    if (landmark) {
      if (persistentCleared.has(key)) {
        this.setMessage(persistentClaimedMines.has(key) ? `${landmark.name}已经被营地占领。` : `${landmark.name}已经搜索过了。`);
        return;
      }
      this.startEncounter(key, tile ?? '', landmark.name, landmark.danger);
      return;
    }
  ...
  private startEncounter(key: string, tile: string, name: string, danger: number): void {
    ...
    this.layoutEncounterPanel();
    this.encounterBackdrop?.setVisible(true);
    this.encounterPanel?.setVisible(true);
    this.refreshEncounter();
  }

  private closeEncounterUi(): void {
    this.encounterBackdrop?.setVisible(false);
  ...
  private fleeEncounter(): void {
    ...
    this.encounter = undefined;
    this.closeEncounterUi();
  }

  private winEncounter(): void {
    if (!this.encounter) return;
    const e = this.encounter;
    const loot = this.generateLoot(e.tile, e.danger);
  ...
  private lootName(key: string): string {
    const names: Record<string, string> = {
      wood: '木材', fur: '毛皮', meat: '肉', leather: '皮革', curedMeat: '熏肉', iron: '铁', coal: '煤', sulphur: '硫磺',
      steel: '钢', medicine: '药剂', scales: '鳞片', teeth: '牙齿', cloth: '布料', bullets: '子弹',
    };
  ...
  private refreshHud(): void {
    ...
    this.hudText?.setText(
      `位置 ${this.px},${this.py} · 距营地 ${Math.abs(this.px) + Math.abs(this.py)}    熏肉 ${this.supplies.curedMeat}    水 ${this.water}/${this.maxWater}\n` +
