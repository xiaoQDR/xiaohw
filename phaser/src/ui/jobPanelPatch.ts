...
type JobId = 'gatherer' | 'hunter' | 'explorer' | 'tanner' | 'charcutier' | 'steelworker' | 'armourer';
type BuildingId = 'lodge' | 'tannery' | 'smokehouse' | 'steelworks' | 'armoury';

interface JobDef { id: JobId; name: string; description: string; requires?: BuildingId; requiresName?: string; }
interface JobState {
...
function refresh(scene: BuildScene, state: JobState): void {
  for (const job of JOBS) {
    ...
    if (!status) continue;
    if (hasBuilding(scene, job.requires)) status.setText(job.id === 'gatherer' ? '默认岗位' : '已解锁').setColor('#a7c893');
    else status.setText(`需 ${job.requiresName}`).setColor('#8e9689');
  }
}
...
function adjust(scene: BuildScene, state: JobState, job: JobDef, delta: number): void {
  if (!hasBuilding(scene, job.requires)) {
    (scene as unknown as { showToast?: (message: string) => void }).showToast?.(`需要先建造${job.requiresName}`);
    return;
  }
  if (delta > 0) {
    if (job.id === 'gatherer') {
...
function createUi(scene: BuildScene): JobState {
  ...
  const state: JobState = {
    ...
    counts: { gatherer: 0, hunter: 0, explorer: 0, tanner: 0, charcutier: 0, steelworker: 0, armourer: 0 },
  ...
  JOBS.forEach((job, index) => {
    ...
    const stateText = scene.add.text(110, y, '', { fontFamily: 'system-ui, sans-serif', fontSize: '15px', color: '#87977f' }).setOrigin(1, 0.5);
    const minus = scene.add.rectangle(210, y, 52, 46, 0x465847, 1).setStrokeStyle(1, 0x71836b, 1).setInteractive({ useHandCursor: true });
    const minusText = scene.add.text(210, y - 1, '−', { fontSize: '29px', color: '#ffffff' }).setOrigin(0.5);
    const count = scene.add.text(280, y, '0', { fontFamily: 'system-ui, sans-serif', fontSize: '25px', color: '#fff0bd', fontStyle: 'bold' }).setOrigin(0.5);
    const plus = scene.add.rectangle(350, y, 52, 46, 0x536a50, 1).setStrokeStyle(1, 0x809476, 1).setInteractive({ useHandCursor: true });
    const plusText = scene.add.text(350, y - 1, '+', { fontSize: '28px', color: '#ffffff' }).setOrigin(0.5);
