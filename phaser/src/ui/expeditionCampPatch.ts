...
type Supplies = Record<Consumable, number>;
...
const GEAR_WEIGHT: Record<string, number> = {
  ...
  lArmour: 2, iArmour: 5, sArmour: 8, waterskin: 1, cask: 2, waterTank: 5,
};
...
function inventoryCount(scene: BuildScene, id: string): number {
  const made = Number(crafted(scene)[id] ?? 0);
...
function safeDistance(state: ExpeditionState): number {
  return Math.max(0, Math.floor(Math.min(state.supplies.curedMeat * 2, waterCapacity(state))));
}
function normalize(scene: BuildScene, state: ExpeditionState): void {
  (Object.keys(state.selected) as GearSlot[]).forEach((slot) => {
    const id = state.selected[slot];
    if (id && !owns(scene, id)) state.selected[slot] = null;
  });
...
function nextOwned(scene: BuildScene, slot: GearSlot, current: string | null): string | null {
  ...
  const list: Array<string | null> = [null, ...owned];
...
function startExpedition(scene: BuildScene, state: ExpeditionState): void {
  normalize(scene, state);
  const check = canDepart(scene, state);
  if (!check.ok) { showToast(scene, check.reason); return; }

  (Object.keys(state.supplies) as Consumable[]).forEach((key) => {
    setResource(scene, key, getResource(scene, key) - state.supplies[key]);
  });
  ...
  setPrivate(scene, 'activeExpedition', payload);
  scene.scene.launch('ExpeditionScene', payload);
  scene.scene.sleep();
}

function inventorySummary(scene: BuildScene): string {
  return (Object.keys(SLOT_OPTIONS) as GearSlot[]).map((slot) => {
    const total = SLOT_OPTIONS[slot].reduce((sum, id) => sum + inventoryCount(scene, id), 0);
    return `${SLOT_NAMES[slot]} ${total}`;
  }).join('   ');
}
function refresh(scene: BuildScene, state: ExpeditionState): void {
  ...
  (Object.keys(state.slotTexts) as GearSlot[]).forEach((slot) => {
    ...
    value.setText(id ? `${GEAR_NAMES[id]}  ×${inventoryCount(scene, id)}  >` : owned.length ? `未装备 · 可选 ${owned.length} 种  >` : '未拥有');
    value.setColor(state.active ? '#8f998d' : owned.length ? '#f4e3b0' : '#7f897d');
  });
  (Object.keys(state.supplies) as Consumable[]).forEach((key) => {
    state.supplyTexts[key].setText(String(state.supplies[key]));
    state.supplyOwnTexts[key].setText(`营地 ${Math.floor(getResource(scene, key))}`);
  });
  const weight = totalWeight(state);
  const capacity = bagCapacity(state);
  state.summaryText.setText(`负重 ${weight} / ${capacity}    水 ${waterCapacity(state)}    生命 ${maxHp(state)}    攻击 ${attack(state)}\n预计安全探索：约 ${safeDistance(state)} 格`);
  state.summaryText.setColor(weight > capacity ? '#e39a8f' : '#dce5d0');
  const check = canDepart(scene, state);
  state.warningText.setText(state.active ? '你正在远征中。' : check.ok ? '准备完成，可以出发。' : check.reason);
  state.warningText.setColor(state.active || check.ok ? '#a9c998' : '#d6a28f');
  state.departText.setText('出发');
  state.departBg.setFillStyle(check.ok ? 0x6b7f4c : 0x454c45, 1);
  state.departText.setColor(check.ok ? '#fff4d6' : '#9ca59a');
}
...
function setOpen(scene: BuildScene, open: boolean): void {
  ...
  state.open = open; state.panel.setVisible(open); if (open) refresh(scene, state);
}
function createSlot(scene: BuildScene, panel: Phaser.GameObjects.Container, y: number, labelText: string) {
  const label = scene.add.text(-335, y - 13, labelText, { fontFamily: 'system-ui, sans-serif', fontSize: '21px', color: '#d5dfca', fontStyle: 'bold' });
  const bg = scene.add.rectangle(190, y, 300, 54, 0x3a473b, 1).setStrokeStyle(1, 0x697967).setInteractive({ useHandCursor: true });
  ...
  panel.add([label, bg, value]); return { bg, value };
}
function createSupplyRow(scene: BuildScene, panel: Phaser.GameObjects.Container, y: number, labelText: string) {
  const label = scene.add.text(-335, y - 13, labelText, { fontFamily: 'system-ui, sans-serif', fontSize: '21px', color: '#d5dfca', fontStyle: 'bold' });
  ...
  panel.add([label, own, minus, minusText, value, plus, plusText]); return { minus, plus, value, own };
}
...
function createPanel(scene: BuildScene): ExpeditionState {
  ...
  const bg = scene.add.rectangle(0, 0, 900, 1360, 0x202820, 0.995).setStrokeStyle(3, 0x829270).setInteractive();
  const title = scene.add.text(-370, -610, '远征准备', { fontFamily: 'system-ui, sans-serif', fontSize: '38px', color: '#fff0d2', fontStyle: 'bold' });
  const sub = scene.add.text(-370, -560, '玩家本人单独出发。装备和营地资源会直接带入荒野大地图。', { fontFamily: 'system-ui, sans-serif', fontSize: '18px', color: '#aebba5', wordWrap: { width: 680 } });
  const closeBg = scene.add.circle(370, -600, 30, 0x4b5849, 1).setInteractive({ useHandCursor: true });
  const closeText = scene.add.text(370, -601, '×', { fontSize: '34px', color: '#fff' }).setOrigin(0.5);
  const playerBg = scene.add.rectangle(0, -465, 760, 96, 0x2b352c, 1).setStrokeStyle(2, 0x536453);
  ...
  const gearBg = scene.add.rectangle(0, -180, 760, 410, 0x2b352c, 1).setStrokeStyle(2, 0x536453);
  ...
  const weapon = createSlot(scene, panel, -255, '武器'); const armour = createSlot(scene, panel, -185, '护甲');
  const bag = createSlot(scene, panel, -115, '背包'); const water = createSlot(scene, panel, -45, '水具');
  const supplyBg = scene.add.rectangle(0, 180, 760, 250, 0x2b352c, 1).setStrokeStyle(2, 0x536453);
  ...
  const food = createSupplyRow(scene, panel, 135, '熏肉'); const medicine = createSupplyRow(scene, panel, 197, '药剂'); const bullets = createSupplyRow(scene, panel, 259, '子弹');
  const summaryBg = scene.add.rectangle(0, 425, 760, 150, 0x273027, 1).setStrokeStyle(1, 0x4f604f);
  ...
  const departBg = scene.add.rectangle(0, 570, 360, 72, 0x454c45, 1).setStrokeStyle(2, 0x7a8c70).setInteractive({ useHandCursor: true });
  ...
  const state: ExpeditionState = {
    ...
    selected: { weapon: null, armour: null, bag: null, water: null }, supplies: { curedMeat: 0, medicine: 0, bullets: 0 },
  ...
  const bindSlot = (slot: GearSlot, object: Phaser.GameObjects.Rectangle) => object.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, e: Phaser.Types.Input.EventData) => {
    e.stopPropagation(); if (state.active) return;
    if (!ownedForSlot(scene, slot).length) { showToast(scene, `工坊尚未制作可用${SLOT_NAMES[slot]}`); return; }
    state.selected[slot] = nextOwned(scene, slot, state.selected[slot]); refresh(scene, state);
  });
  ...
  departBg.on('pointerdown', (_p: Phaser.Input.Pointer, _x: number, _y: number, e: Phaser.Types.Input.EventData) => { e.stopPropagation(); startExpedition(scene, state); });
  ...
  setPrivate(scene, 'finishExpeditionFromWorld', (remaining: Supplies) => {
    (Object.keys(remaining) as Consumable[]).forEach((key) => setResource(scene, key, getResource(scene, key) + Math.max(0, Number(remaining[key] ?? 0))));
    refreshResources(scene);
    state.active = false;
    state.open = false;
    state.panel.setVisible(false);
    state.supplies = { curedMeat: 0, medicine: 0, bullets: 0 };
    setPrivate(scene, 'activeExpedition', undefined);
    refresh(scene, state);
    showToast(scene, '你已返回营地，剩余补给已放回库存');
  });
  layout(scene, state); refresh(scene, state); return state;
}
...
export function installExpeditionCampPatch(): void {
  ...
  const originalCreate = proto.create; const originalUpdate = proto.update;
  proto.create = function patchedCreate(this: BuildScene, ...args: any[]) { const result = originalCreate.apply(this, args); createPanel(this); return result; };
  proto.update = function patchedUpdate(this: BuildScene, ...args: any[]) {
    const result = originalUpdate.apply(this, args); const state = states.get(this);
