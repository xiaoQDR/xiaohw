...
function refreshBuildMenu(scene: BuildScene): void {
  const fn = getPrivate<() => void>(scene, 'refreshBuildMenuNow');
  fn?.();
}
...
function createUi(scene: BuildScene): DebugState {
  ...
  const buttonBg = scene.add.rectangle(0, 0, 250, 66, 0x4c2d2d, 0.97).setStrokeStyle(2, 0xb88172, 1).setInteractive({ useHandCursor: true });
  const buttonLabel = scene.add.text(0, 0, 'DEBUG 资源', { fontFamily: 'system-ui, sans-serif', fontSize: '22px', color: '#fff1dc', fontStyle: 'bold' }).setOrigin(0.5);
