...
type ResourceKey = 'wood' | 'meat' | 'fur' | 'leather' | 'curedMeat' | 'iron' | 'coal' | 'steel' | 'sulphur' | 'bullets';
type SupportedJob = 'tanner' | 'charcutier' | 'steelworker' | 'armourer';
type BuildingId = 'tannery' | 'smokehouse' | 'steelworks' | 'armoury';
type Phase = 'idle' | 'walkingIn' | 'inside';

type WorkerRuntime = { phase: Phase; jobVersion: number; token: number; nextCompleteAt: number; waitingForInput: boolean };
type JobConfig = {
  job: SupportedJob;
  building: BuildingId;
  tint: number;
...
type BuildingBadge = { container: Phaser.GameObjects.Container; title: Phaser.GameObjects.Text; status: Phaser.GameObjects.Text };

const JOBS: JobConfig[] = [
  { job: 'tanner', building: 'tannery', tint: 0x9a7650, workMs: 10_000, input: { fur: 5 }, inputLabel: '毛皮 -5', output: { key: 'leather', amount: 1, label: '皮革' } },
  { job: 'charcutier', building: 'smokehouse', tint: 0x9b5848, workMs: 10_000, input: { meat: 5, wood: 5 }, inputLabel: '肉 -5 · 木材 -5', output: { key: 'curedMeat', amount: 1, label: '熏肉' } },
  { job: 'steelworker', building: 'steelworks', tint: 0x7e8586, workMs: 12_000, input: { iron: 1, coal: 1 }, inputLabel: '铁 -1 · 煤 -1', output: { key: 'steel', amount: 1, label: '钢' } },
  { job: 'armourer', building: 'armoury', tint: 0x6f7778, workMs: 12_000, input: { steel: 1, sulphur: 1 }, inputLabel: '钢 -1 · 硫磺 -1', output: { key: 'bullets', amount: 1, label: '子弹' } },
];
...
function addResource(scene: BuildScene, key: ResourceKey, amount: number): void { setPrivate(scene, key, Math.max(0, getResource(scene, key) + amount)); }
...
function getBuilding(scene: BuildScene, id: BuildingId): Phaser.GameObjects.Image | undefined {
  return (getPrivate<Array<{ id: string; sprite: Phaser.GameObjects.Image }>>(scene, 'placed') ?? []).find((building) => building.id === id)?.sprite;
}
function getRuntime(worker: Phaser.GameObjects.Image): WorkerRuntime {
  let runtime = runtimes.get(worker);
  if (!runtime) { runtime = { phase: 'idle', jobVersion: -1, token: 0, nextCompleteAt: 0, waitingForInput: false }; runtimes.set(worker, runtime); }
  return runtime;
}
...
function showOutput(scene: BuildScene, building: Phaser.GameObjects.Image, config: JobConfig): void {
  const popup = scene.add.text(building.x, building.y - building.displayHeight * 0.62, `+${config.output.amount} ${config.output.label}`, {
    fontFamily: 'system-ui, sans-serif', fontSize: '22px', color: '#fff0b0', fontStyle: 'bold', stroke: '#49362a', strokeThickness: 4,
...
function resetWorker(scene: BuildScene, worker: Phaser.GameObjects.Image, runtime: WorkerRuntime): void {
  runtime.token += 1; runtime.phase = 'idle'; runtime.nextCompleteAt = 0; runtime.waitingForInput = false;
  scene.tweens.killTweensOf(worker);
  if (worker.active) { worker.setVisible(true); worker.clearTint(); }
}
function beginProduction(scene: BuildScene, worker: Phaser.GameObjects.Image, config: JobConfig, runtime: WorkerRuntime): void {
  ...
  if (!payInputs(scene, config)) { runtime.waitingForInput = true; runtime.nextCompleteAt = scene.time.now + 1000; return; }
  runtime.waitingForInput = false; runtime.nextCompleteAt = scene.time.now + config.workMs;
}
function enterBuilding(scene: BuildScene, worker: Phaser.GameObjects.Image, config: JobConfig): void {
  ...
  if (!building) return;
  runtime.phase = 'walkingIn'; runtime.token += 1;
  const token = runtime.token; const version = Number(worker.getData('jobVersion') ?? 0); runtime.jobVersion = version;
  scene.tweens.killTweensOf(worker); worker.setVisible(true).setTint(config.tint);
  scene.tweens.add({
    targets: worker,
    x: building.x + Phaser.Math.Between(-26, 26), y: building.y + Math.max(20, building.displayHeight * 0.24),
    duration: Phaser.Math.Between(1700, 2300), ease: 'Sine.InOut',
    onComplete: () => {
      if (!worker.active || worker.getData('job') !== config.job || Number(worker.getData('jobVersion') ?? 0) !== version || runtime.token !== token) return;
      runtime.phase = 'inside'; worker.setVisible(false); beginProduction(scene, worker, config, runtime);
    },
  });
}
function tickInsideWorker(scene: BuildScene, worker: Phaser.GameObjects.Image, config: JobConfig, runtime: WorkerRuntime): void {
  const version = Number(worker.getData('jobVersion') ?? 0);
  if (runtime.jobVersion !== version) { resetWorker(scene, worker, runtime); return; }
  worker.setVisible(false);
  if (scene.time.now < runtime.nextCompleteAt) return;
  if (runtime.waitingForInput) { beginProduction(scene, worker, config, runtime); return; }
  const building = getBuilding(scene, config.building);
  if (!building) { resetWorker(scene, worker, runtime); return; }
  addResource(scene, config.output.key, config.output.amount); refresh(scene); showOutput(scene, building, config); beginProduction(scene, worker, config, runtime);
}
function getBadgeMap(scene: BuildScene): Map<BuildingId, BuildingBadge> {
  let map = badges.get(scene); if (!map) { map = new Map(); badges.set(scene, map); } return map;
}
function ensureBadge(scene: BuildScene, config: JobConfig, building: Phaser.GameObjects.Image): BuildingBadge {
  const map = getBadgeMap(scene); let badge = map.get(config.building); if (badge) return badge;
  const bg = scene.add.rectangle(0, 0, 280, 82, 0x253026, 0.94).setStrokeStyle(2, 0x768765, 1);
  const title = scene.add.text(0, -20, '', { fontFamily: 'system-ui, sans-serif', fontSize: '18px', color: '#fff1cd', fontStyle: 'bold' }).setOrigin(0.5);
  const status = scene.add.text(0, 16, '', { fontFamily: 'system-ui, sans-serif', fontSize: '16px', color: '#c7d4ba' }).setOrigin(0.5);
  ...
  badge = { container, title, status }; map.set(config.building, badge); return badge;
}
function refreshBadges(scene: BuildScene): void {
  ...
  for (const config of JOBS) {
    const building = getBuilding(scene, config.building); const existing = getBadgeMap(scene).get(config.building);
    if (!building) { existing?.container.setVisible(false); continue; }
    const assigned = workers.filter((worker) => worker.active && worker.getData('job') === config.job);
    if (assigned.length === 0) { existing?.container.setVisible(false); continue; }
    ...
    if (inside.length === 0) { badge.status.setText('工人正在进入建筑').setColor('#d7cba7'); continue; }
    if (waiting && inside.every((runtime) => runtime.waitingForInput)) { badge.status.setText(`材料不足 · ${config.inputLabel}`).setColor('#e4a48d'); continue; }
    const active = inside.filter((runtime) => !runtime.waitingForInput);
    const readyAt = active.length > 0 ? Math.min(...active.map((runtime) => runtime.nextCompleteAt)) : scene.time.now;
    badge.status.setText(`CD ${Math.max(0, Math.ceil((readyAt - scene.time.now) / 1000))}s · ${config.inputLabel}`).setColor('#c7d4ba');
  }
}
function tickWorkers(scene: BuildScene): void {
  ...
  for (const worker of workers) {
    ...
    const config = configs.get(String(worker.getData('job') ?? 'gatherer'));
    const runtime = getRuntime(worker); const version = Number(worker.getData('jobVersion') ?? 0);
    if (!config) { if (runtime.phase !== 'idle' || !worker.visible) resetWorker(scene, worker, runtime); continue; }
...
export function installBuildingWorkerPatch(): void {
  ...
  proto.update = function patchedUpdate(this: BuildScene, ...args: any[]) {
    const result = originalUpdate.apply(this, args); tickWorkers(this); return result;
  };
}
